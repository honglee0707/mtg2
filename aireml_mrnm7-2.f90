...
subroutine aireml_rnm_m_h2 (mbin,yidx,yv,rn,rnm,trn,pedn,ttn,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl12,fl24,xmm,nit,conv,thn)        
...
real::mbin(mn,pedn,pedn),yv(rn,tn),tyv(trn,1),rv1
...
double precision::blup_ebv(pedn,100,mn),beta(tfn,1)!total sum k (pol oreder)<100
...
double precision:: t1_cpu, t2_cpu, elapsed_cpu_secs,t3_cpu,t4_cpu
!***********************************************************************
...
double precision,allocatable::tmp_v(:,:),tmp_vcva2(:,:),tmp_v2(:,:)
...
integer::idum,idum2,ii,jj,kk,kk2,oo,ii2,jj2,xi2,i2,j2
...
print*,''
  ...
  if (file_exist) then
    ...
    do xi=2,ttn      !because ttn = tn
      do jj=1,(xi-1)
        read(44,*,iostat=io)cdum,vcve(xi,jj)
      end do
    end do
  ...
  if (file_exist) then
    ...
    do xi=2,ttn      !because ttn = tn
      do jj=1,(xi-1)
        read(51,*,iostat=io)cdum,fix_vcve(xi,jj)
        fix_vcve(jj,xi)=fix_vcve(xi,jj)
      end do
    end do
    do wi=1,mn
      do ii=1,ttn
  ...
  !polynomial components not same for all multiple random effects
    ...
    do i=1,tn
      do j=1,tn
        if (fix_vcve(i,j) .ne. -99999) vcve(i,j)=fix_vcve(i,j)
      end do
    end do
    do wi=1,mn
      do ii=1,ttn
  ...
  !pause
    ...
    !  end do
    !end do
    call dgemm ('N','T',tfn,trn,tfn,1.0D0,xvmx,tfn,xm,trn,0.0D0,xvm2,tfn)
    ...
    !$OMP END PARALLEL DO
    !call dgemm ('N','T',trn,tfn,trn,1.0D0,pm,trn,xvm2,tfn,0.0D0,xvm3,trn)
   ...
   !print*,'beta:',beta
    ...
    !do i=1,trn
    !  do j=i,trn
    !    !v1=dot_product(xvm3(i,:),xvm(:,j))
    !    !pm(i,j)=pm(i,j)-v1
    !    pm(i,j)=pm(i,j)-dot_product(xvm3(i,:),xvm(:,j))
    !    pm(j,i)=pm(i,j)
    !  end do
    !end do
    !!$OMP END PARALLEL DO
    ...
    do i=1,trn
      do j=1,trn
        ...
        do k=1,tfn
          ...
          !pm(i,j)=pm(i,j)-xvm3(i,k)*xvm(k,j)
          !call sleepqq(1) 
          !call beepqq(1,1) 
        end do
        pm(i,j)=pm(i,j)-v1
    ...
    LKH=-0.5*(LA+LG+v2)
    !print*,LA,LG,v2,vcve,LKH,LKHP  !ypyf
    !pause
    call cpu_time (t2_cpu)
    ...
    do xi=2,ttn           !because ttn=tn
      do jj=1,(xi-1)
        PRINT '(a7,100f14.4)','cove',vcve(xi,jj)
      end do
    end do
    ...
    do k=1,mn !no. random effects
      ...
      do ii=1,ttn
        ...
        do xi=1,m_tnk(ii,k)   !check
          ...
          !do i=1,rnm(ii)
          !  do j=1,rnm(ii)
          !    !tmp_vcva(i,j)=phi(i,xi)*phi(j,xi)
          !    pig_vcva(k,mm+i,mm+j)=phi(i,xi)*phi(j,xi)
          !  end do
          !end do
          !!$OMP END PARALLEL DO 
...
&                   +mbin(k2,yidx(ui,i),yidx(yi,j))*pig_vcva(k2,trnx-rnm(ui)+i,trny-rnm(yi)+j)
    ...
    !print*,'derivatives for dgemm - time:',real(t4_cpu-t3_cpu)
          !idum=tn
          idum=tn+(ttn**2/2-ttn/2)
          do ll=1,mn
            do kk=1,ttn
...
50        continue
          ...
          !for Vk x cove ***************************************************
          nbc2=0  !for no. block for cove, i.e. 1:(2,1), 2:(3,1), 3:(3,2), ... 
          trnv=rnm(1)
          do vi=2,tn
...
&                     +mbin(k2,yidx(ui,i),yidx(yi,j))*pig_vcva(k2,trnx-rnm(ui)+i,trny-rnm(yi)+j)
    ...
    !call cpu_time (t4_cpu)
    !print*,'derivatives for vg x vg pm - time:',real(t4_cpu-t3_cpu)
