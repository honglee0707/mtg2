subroutine genedrop(nmarkers,neff,ngen,nch,chl,seed2,mr,spf,empsz,sim_coal_file)
...
integer::rseed,nmarkers,neff,ngen,seed2,spf,prn,empsz
parameter(prn=15)
...
integer::neff3,nmal2,nfem2,kkk,popmal,popfem,nfem,neff2,popsiz,nmal
...
integer::contped(neff*empsz,4),ncont10
...
print*,'#generation #recombination #mutation'
  ...
  do igen=1,ngen   !historical generation starts********************
     print'(*(G0,10X))','',igen,matcn(1,1),mutcn(1,1)
...
147         call random_number(rnd)
   ...
   if (mod(igen,50)==0 .or. igen==ngen) then       !every 100 generation
     do i=1,neff
       do ich=1,nch
  ...
  END do   !igen ~ ngen
  !PRINT*,'mutation - size',neff3,'size',popsiz
  !pause
...
1471   continue
  ...
  do k=1,ncont10
    !write(101,'(*(G0,2X))')contped(k,1),contped(k,1),0,0,2,-9,'  ',(contom(k*2-1,midx(j)),' ',contom(k*2,midx(j)),'  ',j=1,midxn)
    write(101,'(2I10,4I3,100000000a2)')contped(k,1),contped(k,1),0,0,contped(k,4),-9,'  ',(contom(k*2-1,midx(j)),' ',contom(k*2,midx(j)),'  ',j=1,midxn)
