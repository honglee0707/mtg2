...
subroutine aireml_m_h2 (mbin,yidx,yv,rn,rnm,trn,pedn,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl24,xmm,nit,conv,wt_res)        
    ...
    if ((LKH>-99999999 .and. LKH<99999999) ) then
      ...
      if (zi==1) then     !the first iteration
        ...
        do xi=1,tn
          ...
          !vcve(xi,xi)=((x2-(x1**2/rnm(xi)))/(rnm(xi)-1))*.9
          !vcva(:,xi,xi)=((x2-(x1**2/rnm(xi)))/(rnm(xi)-1))*.1/mn
...
if (fl11r.ne.'null') then
  ...
  !$OMP PARALLEL DO PRIVATE(vi,zi,k,v1,v2,trnx,trny,yi,j,i)
  do vi=1,tn
    do zi=1,pedn
      do k=1,mn
        v2=0
