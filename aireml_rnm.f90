...
subroutine aireml_rnm_h (mbin,yidx,yv,rn,rnm,trn,pedn,tn,fnm,tfn,mn,fl4,fl5,fl11,fl12,xmm,nit,conv)        
...
double precision::vcva(mn,trn,trn),pig_vcva(mn,trn,trn),vcve(tn,tn)
double precision::vcve_rsv(tn,tn)
!double precision::blup_ebv(pedn,tn,mn),blup_py(pedn,tn,mn),beta(tfn,1) !,cor
double precision::blup_ebv(pedn,20,mn),beta(tfn,1) ! check assuming that always k (pol oreder) < 20
...
double precision::vcva2(trn,trn),am2(20,20),mm2(trn,20)
...
double precision::wk(trn),xk(trn)
...
am2(20,20)=+ 297699.8
  ...
  !xk=(wk-wk(trn)/2)/(wk(trn)/2)
  xk=wk  !normal polynomial

  !wk=wk-minval(wk)
  !xk=(wk-maxval(wk)/2)/(maxval(wk)/2)
  !print*,minval(xk),maxval(xk) !xk
  !pause
  ...
  do wi=1,mn
    ...
    allocate(am(tnk(wi),tnk(wi)),mm(trn,tnk(wi)),phi(trn,tnk(wi)),km(tnk(wi),tnk(wi)))
    am=am2(1:tnk(wi),1:tnk(wi))
    mm=mm2(1:trn,1:tnk(wi))

    !phi=matmul(mm,am)   !***************************************
    phi=mm     !normal polynomial
  ...
  do wi=1,mn
    ...
    !do i=1,trn
    !  print*,xk(i),phi(i,1:tnk(wi)),vcva(1,i,i)
    !end do
    !pause
      ...
      do i=1,rnm(xi)
        do j=1,rnm(yi)
          do k=1,mn
            ...
            pm(trnx-rnm(xi)+i,trny-rnm(yi)+j)=pm(trnx-rnm(xi)+i,trny-rnm(yi)+j)+mbin(k,yidx(xi,i),yidx(yi,j))*vcva(k,i,j)
          end do
          if (xi.ne.yi) then
  ...
  !pause
   ...
   !print*,'beta:',beta
    ...
    do k=1,mn !no. random effects
      ...
      allocate(am(tnk(k),tnk(k)),mm(trn,tnk(k)),km2(tnk(k),tnk(k)),phi(trn,tnk(k)))
      am=am2(1:tnk(k),1:tnk(k))
      mm=mm2(1:trn,1:tnk(k))
...
&                 +mbin(k2,yidx(ui,i),yidx(yi,j))*pig_vcva(k2,i,j)
                end do
                if (ui.ne.yi) then
...
&                   +mbin(k2,yidx(ui,i),yidx(yi,j))*pig_vcva(k2,i,j)
        ...
        do wi=1,k-1  !check in case multiple random effects
          ...
          allocate(am_2(tnk(wi),tnk(wi)),mm_2(trn,tnk(wi)))
          allocate(km2_2(tnk(wi),tnk(wi)),phi_2(trn,tnk(wi)))

          am_2=am2(1:tnk(wi),1:tnk(wi))
          mm_2=mm2(1:trn,1:tnk(wi))

          !phi_2=matmul(mm_2,am_2)  
          phi_2=mm_2 !normal polynomial  
...
&                       +mbin(k2,yidx(ui,i),yidx(vi,j))*pig_vcva(k2,i,j)
                      end do
                      if (ui.ne.vi) then
...
&                     +mbin(k2,yidx(ui,i),yidx(wi,j))*pig_vcva(k2,i,j)
                    end do
                    if (ui.ne.wi) then
...
&                         +mbin(k2,yidx(wj,i),yidx(wi,j))*pig_vcva(k2,i,j)
                        end do
                        if (wj.ne.wi) then
          ...
          do wi=1,k-1   !check in case of multiple random effects
              ...
              allocate(am_2(tnk(wi),tnk(wi)),mm_2(trn,tnk(wi)),km2_2(tnk(wi),tnk(wi)),phi_2(trn,tnk(wi)))
...
&                       +mbin(k2,yidx(ui,i),yidx(yi2,j))*pig_vcva(k2,i,j)
                      end do
                      if (ui.ne.yi2) then
...
if (fl11.ne.'null') then
  ...
  do k=1,mn
    ...
    allocate(am(tnk(k),tnk(k)),mm(trn,tnk(k)),phi(trn,tnk(k)),km(tnk(k),tnk(k)))
    allocate(km_phi(tnk(k),trn))
    am=am2(1:tnk(k),1:tnk(k))
    mm=mm2(1:trn,1:tnk(k))
    !phi=matmul(mm,am)   !***************************************
    phi=mm   !normal polynomial ***************************************
      ...
      do zi=1,pedn
        ...
        do yi=1,tn
          ...
          do j=1,rnm(yi)
            ...
            v1=v1+mbin(k,yidx(yi,j),zi)*km_phi(xi,j)*py(trny-rnm(yi)+j,1) !check 
          end do
        end do
        blup_ebv(zi,xi,k)=v1
      end do !zi
  ...
  do k=1,mn
    do xi=1,tnk(k)    !check
      do i=1,pedn
        write(43,'(i3,i3,i7,100f24.16)')k,xi,i,blup_ebv(i,xi,k)
      end do
    end do
  end do
  close(43)
