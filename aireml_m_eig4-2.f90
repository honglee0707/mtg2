...
subroutine aireml_m_eig2 (mbin,eval,yidx,yv,rn,rnm,trn,pedn,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,xmm,nit,conv)        
...
double precision::blup_ebv2(pedn,tn,mn),blup_py2(pedn,tn,mn) !,cor
!double precision::blup_r2(pedn,tn,mn),blup_r(pedn,tn,mn) !,cor
...
double precision::xvmx(tfn,tfn),xvm(tfn,trn),xvmy(trn,1) 
...
double precision::vmi2(trn,trn),pm2(trn,trn)
...
print*,''
  ...
  do i=1,pedn     !assuming all ID have record
    ...
    do xi=1,tn
      do yi=1,xi
        !vmi2(pedn*xi-pedn+i,pedn*yi-pedn+i)=tmp_pm(xi,yi)
        !vmi2(pedn*yi-pedn+i,pedn*xi-pedn+i)=tmp_pm(xi,yi)
  ...
  print*,'V inverse done - time:',real(t1_cpu-t2_cpu) !,LA
    ...
    do i=1,tfn
      do yi=1,tn
        do j=1,pedn
          v1=0
          do xi=1,tn
            !v1=v1+xm(pedn*xi-pedn+j,i)*vmi2(pedn*xi-pedn+j,pedn*yi-pedn+j)
            v1=v1+xm(pedn*xi-pedn+j,i)*vmi(j,xi,yi)
            !if (vmi2(pedn*xi-pedn+j,pedn*yi-pedn+j).ne.vmi(j,xi,yi)) then
            !  print*,j,xi,yi
            !  pause
            !end if
          end do
   ...
   !print*,'test  - time:',real(t4_cpu-t3_cpu) 
    ...
    do yi=1,tn
      do i=1,pedn
        v1=0
        do xi=1,tn
          !v1=v1+vmi2(pedn*yi-pedn+i,pedn*xi-pedn+i)*res(pedn*xi-pedn+i,1)
          ...
          do ui=1,(vi-1)
            ...
            if (nbc2<nbc) then  !off diagonal within cove ***************
              ...
              do i=1,rnm(vi)
                ...
                !    v1=v1+pdpy(trnu-rnm(ui)+j,1) !*pig_tmp
                !  end if
                !end do
              ...
              do i=1,rnm(ui)
                ...
                !    v1=v1+pdpy(trnv-rnm(vi)+j,1) !*pig_tmp
                !  end if
                !end do
    ...
    do xi=2,tn
      ...
      do yi=1,xi-1
        ...
        tr1=0
        do i=1,rnm(xi)
          !v10=pm2(trnx-rnm(xi)+i,trny-rnm(xi)+i)     !pm*pig
          v10=pm(i,xi,yi)     !pm*pig
          tr1=tr1+v10
        end do
        do i=1,rnm(yi)
          !v10=pm2(trny-rnm(yi)+i,trnx-rnm(yi)+i)     !pm*pig
    ...
    do k=1,mn !no. random effects
      ...
      do xi=1,tn
        ...
        aim(nb+nb*k-nb+xi,nb+nb*k-nb+xi)=v3
        ...
        do vi=1,xi-1
          ...
          v2=0
          do i=1,rnm(vi)
            v1=pdpy(trnv-rnm(vi)+i,1)*eval(yidx(vi,i),1)   !*pdpy * d
            v2=v2+v1*py(trnv-rnm(vi)+i,1)                  !*py
          end do
        ...
        do wi=1,k-1
          ...
          do vi=2,tn
            ...
            do ui=1,(vi-1)
              ...
              v2=0
              do i=1,rnm(vi)
                v1=pdpy(trnu-rnm(ui)+i,1)*eval(yidx(ui,i),1)     !*pdpy * d
                v2=v2+v1*py(trnv-rnm(vi)+i,1)                    !*py
              end do
              do i=1,rnm(ui)
                v1=pdpy(trnv-rnm(vi)+i,1)*eval(yidx(vi,i),1)     !*pdpy * d
     ...
     trnx=0
      ...
      do xi=2,tn
        ...
        do yi=1,xi-1
          ...
          !print*,v3
          aim(nb+nb*k-nb+tn+nbc,nb+nb*k-nb+tn+nbc)=v3     !check
          ...
          do vi=2,tn
            ...
            do ui=1,(vi-1)
              ...
              v2=0
              do i=1,rnm(vi)
                v1=pdpy(trnu-rnm(ui)+i,1) !*pig_tmp
                v2=v2+v1*py(trnv-rnm(vi)+i,1) !*py
              end do
              do i=1,rnm(ui)
                v1=pdpy(trnv-rnm(vi)+i,1) !*pig_tmp
                v2=v2+v1*py(trnu-rnm(ui)+i,1)  !*py
              end do
...
!print*,'Estimation done - time:',real(t1_cpu-t3_cpu) !,LA
!print*,'Estimation done - time:',real(c2-c1)/rate !,LA
    ...
    do zi=1,pedn
      do k=1,mn
        ...
        do yi=1,tn
          ...
          !  end if
          !end do
          !v1=v1+eval(zi,1)*vcva(k,xi,yi)*py(yi*pedn-pedn+zi,1)  !check 1
          !v2=v2+vcva(k,xi,yi)*py(yi*pedn-pedn+zi,1)
          v1=v1+eval(zi,1)*vcva(k,xi,yi)*py(yi*pedn-pedn+yidx2(zi),1)  !check 1
          v2=v2+vcva(k,xi,yi)*py(yi*pedn-pedn+yidx2(zi),1)
        end do
        blup_ebv(zi,xi,k)=v1
        blup_py(zi,xi,k)=v2
        !blup_r(zi,xi,k)=eval(zi,1)*pm(zi,xi,xi)*eval(zi,1)
      end do !k
    end do !zi
  end do !xi
      ...
      do k=1,mn
        v1=0;v2=0;v3=0
          do j=1,pedn
            ...
            !v3=v3+mbin(k,zi,j)*blup_r(j,xi,k)
          end do
        blup_ebv2(zi,xi,k)=v1
        blup_py2(zi,xi,k)=v2
        !blup_r2(zi,xi,k)=v3    !reliability
      end do !k
    end do !zi
  end do !xi
...
if (fl11r.ne.'null') then
      ...
      do k=1,mn
        ...
        do i=1,pedn
          do xi=1,tn
            ...
            do yi=1,tn
              !v1=v1+eval(zi,1)*vcva(k,xi,yi)*py(yi*pedn-pedn+zi,1)  !check 1
              !v1=v1+eval(zi,1)*vcva(k,xi,yi)*pm(i,xi,yi)
              v1=v1+eval(zi,1)*vcva(k,xi,yi)*pm(yidx2(i),xi,yi)
            end do
            v2=v2+v1*eval(zi,1)
          end do
        end do
 ...
 ! do xi=1,tn
 !   do i=1,pedn
 !     write(46,'(i3,100f20.16)')xi,blup_r2(i,xi,:)
 !   end do
 ! end do
 ! close(46)
