...
subroutine aireml_rrm_eig_m (mbin,eval,yidx,yv,rn,rnm,trn,pedn,ttn,tn,fnm,tfn,mn,fl4,fl5,fl11,fl12,xmm,nit,conv)        
...
INTEGER::n,nped,ix,iz,nb,nb2,nb3,nbc,nbc2,xi,xi2,yi,yi2,ri,zi,wj
...
integer::idum,idum2,ii,ii2,jj,jj2,kk,ll,mm,mm2,oo
...
!do i=2,ttn
!  do j=1,i-1
!    read(46,*)ctnk(i,j,:)       !# covariance (between traits for rrc) term
!    !print*,ctnk(i,j,:)
!  end do
!end do
close(46)
      ...
      do jj=1,ii-1
        ...
        do i=1,m_tnk(ii,k)
          do j=1,sub_tn(jj)
            ...
            gr_km_phi(i2,j2)=km_phi(i,j)
            !print*,gr_km_phi(i2,j2),i2,j2,i,j,ii,jj
            !pause
          end do
        end do
      ...
      do xi=1,sum(m_tnk(1:ttn,k))    !check
        do zi=1,pedn
          ...
          do yi=1,tn
            ...
            !print*,v1,eval(zi,k),gr_km_phi(xi,yi),py(trny-rnm(yi)+yidx2(zi),1),yidx2(zi)
            !pause
          end do
          blup_ebv(zi,xi,k)=v1
        end do !zi
